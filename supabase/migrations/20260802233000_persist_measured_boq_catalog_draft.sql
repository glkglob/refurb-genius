-- Ticket 4C2E-B2D — Atomic draft catalogue package persistence RPC
--
-- Adds:
--   public.persist_measured_boq_catalog_draft (SECURITY DEFINER, service_role EXECUTE only)
--
-- Does NOT:
--   reopen table DML for service_role / JWT roles
--   create publish/retire/rollback RPCs
--   create an active-revision pointer
--   accept actor identity or caller-supplied ok/production authority

-- ────────────────────────────────────────────────────────────────────
-- 1. Atomic draft persistence
-- ────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  -- ── Transaction-scoped advisory lock on package identity ─────────
...
COMMENT ON FUNCTION public.persist_measured_boq_catalog_draft(
  ...
  'B2D atomic draft catalogue package persistence. service_role EXECUTE only; table DML remains revoked.';
...
-- Pin ownership to the trusted table owner so SECURITY DEFINER writes
-- satisfy measured_boq_catalog_is_trusted_lifecycle_owner() for entry inserts.
