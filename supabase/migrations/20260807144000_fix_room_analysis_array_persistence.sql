-- P0-PHOTO-ANALYZE production hotfix
-- room_analyses.visible_issues and recommended_works are text[] in production.
-- replace_project_room_analyses previously attempted to insert jsonb values.
-- This forward migration preserves the R4 authority/locking contract and converts
-- incoming JSON arrays to canonical text[] values before INSERT.
...
COMMENT ON FUNCTION public.replace_project_room_analyses(uuid, jsonb) IS
  'Atomic room_analyses replacement. Project FOR UPDATE serializes with photo catalogue mutations. Complete catalogue required. JSON array fields are converted to text[] for room_analyses persistence.';
