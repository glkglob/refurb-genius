...
> **IA-0 workflow contract (LOCKED):** [workflow/ia-0-workflow-authority-spec.md](./workflow/ia-0-workflow-authority-spec.md) — governing Photos → Analysis → Redesign → Estimate → Export for IA-1 through IA-10.
...
| **IA-2** | Canonical Next-Action Resolver                                  | **Planned / Next (unblocked)** | Governed by IA-0 §9 including `reconcile_scope` / `view_stage_progress`; **not started** |
...
**IA-2 MUST NOT begin until explicitly authorised** (IA-0-CL1 clarification must be on main first).
