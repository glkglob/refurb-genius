...
// AO-1M6 Extract DealAnalysisCard Mutation Ownership In Progress (local implementation pending verification)
...
export const MIGRATION_CANDIDATES: MigrationCandidate[] = [
  ...
  },
  {
    ...
    evidence: {
      productionImporters: 1,
      notes:
        "AO-1M6 In Progress — implemented locally, pending independent verification. One child slice of Active AO-1 (does not complete AO-1; does not redesign AI/provider/prompt/model; does not add analysis persistence; does not migrate deterministic DealIntakeForm analyzeDeal; does not address shell auth.signOut or AO-1Z). DealAnalysisCard consumes useAnalyzeDealOpportunity through @/features/deal-copilot. The card retains opportunityId prop, explicit click trigger, rendering, copy-to-clipboard, copied local state, button labels, pending disable, inline error rendering, and analysis-result rendering. The presentation hook owns useMutation lifecycle, analyzeDealServerFn({ data: variables }) invocation, AnalyzeDealOpportunityVariables { opportunityId, promptContext? }, DealAnalysis result propagation, error/pending state, and success-only trackEvent(deal_analyzed) with no payload. analyzeDealServerFn remains in src/serverFns/dealAnalysis.ts (unchanged): POST, Zod validation, requireUser, rateLimitKeyForUser deal-analysis, server Supabase deal_opportunities SELECT eq id single, RLS, Opportunity not found, dynamic runDealAnalysis, DealAnalysis return. No QueryClient operations, no browser Supabase in hook, no analysis write, no automatic invocation. Public API extends @/features/deal-copilot with useAnalyzeDealOpportunity + AnalyzeDealOpportunityVariables while retaining AO-1M5 update exports; does not export serverFn/adapter/schema. Lexical seal tests/invariants/deal-analysis-card-mutation-presentation.invariant.test.ts. Legacy baseline shrinks obsolete DealAnalysisCard|@/lib/analytics edge. Schema/migrations/RLS/dependency files unchanged. AO-1 remains Active; AO-1M5 remains Completed. Pending: independent verification, commit, push, exact-SHA CI, governance close-out.",
    },
  },
];
