...
// AO-1M5 Extract Deal Opportunity Update Mutation Ownership In Progress (local implementation pending verification)
...
export const MIGRATION_CANDIDATES: MigrationCandidate[] = [
  ...
  },
  {
    ...
    evidence: {
      ...
      notes:
        'AO-1M5 In Progress — implemented locally, pending independent verification. One child slice of Active AO-1 (does not complete AO-1; does not extract opportunity save/delete; does not migrate opportunity list/detail reads; does not introduce opportunityKeys factory; does not server-migrate update; does not address AO-1M6 DealAnalysisCard or shell signOut). The deal-opportunity edit route consumes useUpdateOpportunity through @/features/deal-copilot. The route retains form state, UI status subset, mutation variables, inline error handling, success navigation, and current pending/duplicate-submit behaviour. The presentation hook owns useMutation, repository delegation, mapped DealOpportunity result propagation, success-only invalidation of ["opportunities"], and standard pending/error state. The feature repository owns browser Supabase, domain-to-database partial update mapping, unconditional updated_at, deal_opportunities UPDATE, opportunity ID filter, select().single(), database-row to domain mapping, and persistence error propagation. No optimistic update, snapshots or rollback are introduced. No auth.getUser, server function or application ownership query is introduced. RLS remains the ownership authority. The public mutation contract remains { id, updates }. The partial update continues to preserve zero and null values and omit undefined values. src/hooks/useOpportunities.ts retains list, detail, save and delete behaviour, but no longer owns useUpdateOpportunity. The dead opportunityStore update path and updateDealOpportunity export are removed. DealAnalysisCard and analyzeDealServerFn remain reserved for AO-1M6. Lexical seal tests/invariants/deal-opportunity-update-presentation.invariant.test.ts. Schema/migrations/RLS/dependency files unchanged. AO-1 remains Active; AO-1M4 remains Completed. Pending: independent verification, commit, push, exact-SHA CI, governance close-out.',
    },
  },
];
