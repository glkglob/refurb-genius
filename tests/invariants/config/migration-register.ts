...
// AO-1S1 Extract Shell Sign-out Ownership In Progress (local implementation pending verification)
...
export const MIGRATION_CANDIDATES: MigrationCandidate[] = [
  ...
  },
  {
    ...
    evidence: {
      ...
      notes:
        "AO-1S1 In Progress — implemented locally, pending independent verification. One child slice of Active AO-1 (does not complete AO-1; does not migrate useAuth.signOut or settings delete-account; does not redesign AuthProvider/cache lifecycle; does not edit frozen src/lib/auth.ts; does not address AO-1Z). Sidebar and MobileTopBar consume useSignOut through @/features/auth. Both retain await signOut() then navigate({ to: '/' }), labels, and absence of pending/error/toast/analytics/QueryClient. Private signOutSession delegates once to auth.signOut() with no options. Public API exports useSignOut (+ UseSignOutResult); does not export signOutSession. Lexical seal tests/invariants/shell-auth-signout-ownership.invariant.test.ts. Legacy baseline shrinks obsolete Sidebar and MobileTopBar | @/lib/auth edges. AO-1 remains Active; AO-1M6 remains Completed. Pending: independent verification, commit, push, exact-SHA CI, governance close-out.",
    },
  },
];
