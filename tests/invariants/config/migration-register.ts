...
// AO-1M3 Extract Gallery Upsert Mutation Ownership In Progress (local implementation pending verification)
...
export const MIGRATION_CANDIDATES: MigrationCandidate[] = [
  ...
  },
  {
    ...
    evidence: {
      productionImporters: 0,
      notes:
        "AO-1M3 In Progress — implemented locally, pending independent verification. One child slice of Active AO-1 (does not complete AO-1; does not server-migrate gallery publish; does not extract cover upload; does not migrate public gallery reads; does not address R-M3–R-M6 stage/opportunity/DealAnalysisCard/shell signOut). Persistence authority: galleryRepository.upsertGalleryProject → public_gallery_projects upsert onConflict project_id select * single; payload project_id/created_by/slug=projectId + is_public/featured/description/cover_image_url + title input.title ?? Untitled Project; error log + throw Error(message). Canonical presentation authority: useUpsertGalleryProject(projectId) from @/features/gallery; auth.getUser gate You must be signed in; optimistic cancelQueries + setQueryData merge/fallback against galleryKeys.byProject → [projects, projectId, gallery]; onError restore previous (including undefined); onSettled invalidateQueries same key non-exact fire-and-forget; no public-list/byId/leads invalidation. PublishToGallery retains form state, public/featured toggles with override save, title/description trim, cover upload via uploadGalleryCoverImage, gallery/leads reads, public link, stats, toast.success Gallery settings saved / toast.error err.message || Failed to save gallery settings. Transitional src/hooks/useGallery.ts deleted; hooks freeze allowlist and legacy import baseline shrink. Lexical seal tests/invariants/gallery-publish-mutation-presentation.invariant.test.ts. Stage/opportunity/DealAnalysisCard/shell residuals unchanged. AO-1 remains Active; AO-1M2 remains Completed. Pending: independent verification, commit, push, CI, close-out.",
    },
  },
];
