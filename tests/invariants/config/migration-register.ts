...
// AO-1K1 Unify Estimate Query-Key Authority In Progress (local implementation pending verification)
...
export const MIGRATION_CANDIDATES: MigrationCandidate[] = [
  ...
  },
  {
    ...
    currentOwner:
      "Canonical room-estimate cache authority: projectKeys.estimateByProject / estimateQueryOptions. AI save invalidation: useSaveAIEstimate (product estimate + financials). AI room read: useRoomEstimate → estimateQueryOptions. Manual save: useSaveEstimateBuilder (unchanged). Persistence: saveAIEstimate (unchanged).",
    targetOwner:
    ...
    evidence: {
      productionImporters: 0,
      notes:
        'AO-1K1 In Progress — implemented locally, pending independent verification. One child slice of Active AO-1 (does not complete AO-1; does not extract AIEstimateBuilder save ownership; does not move estimateQueryOptions/projectKeys; does not unify getLatestProjectEstimate). Canonical room-estimate cache authority established: projectKeys.estimateByProject. Canonical tuple preserved: ["projects", projectId, "estimate"]. Duplicate production key retired: ["room-estimate", projectId]. useRoomEstimate aligned to estimateQueryOptions (spread + enabled: !!projectId; queryFn getLatestRoomEstimate via factory). useSaveAIEstimate now invalidates the canonical product estimate key after successful persistence. useSaveAIEstimate now invalidates project financials after successful persistence to match manual-save cache-coherence behaviour. Estimate invalidation remains project-scoped, success-only, fire-and-forget (void), and non-exact. Financials invalidation remains project-scoped, success-only, fire-and-forget (void), and non-exact. Project isolation preserved. Existing product readers, prefetchers, optimistic writers, and cache-only update hooks remain unchanged. Canonical tuple shape remains unchanged. No cache alias, compatibility bridge, or dual invalidation retained. Estimate repository contracts remain unchanged. Database schema, migrations, RLS, generated types, pricing, VAT, contingency, totals, AI generation, prompts, and models remain unchanged. AIEstimateBuilder save-path ownership remains unchanged and deferred. Production change: src/features/estimate/presentation/hooks/useEstimate.ts only. Focused tests: useEstimate.test.ts. Lexical seal: tests/invariants/estimate-query-key-authority.invariant.test.ts. AO-1 remains Active. Pending: independent verification, commit, push, CI, close-out.',
    },
  },
];
