...
export const MIGRATION_CANDIDATES: MigrationCandidate[] = [
  ...
  },
  {
    ...
    evidence: {
      ...
      productionImporters: 0,
      notes:
        "AO-1H1 Completed. One child slice of Active AO-1 (does not complete AO-1; does not extract estimate tag sync; does not register AO-1H2; does not consolidate dual estimate query keys). Implementation commit b97654d158c8d09fc772c5bf6f304044746d4484 (parent ad21713); subject refactor(floorplan): extract viewer auth and persistence mutations; 12 files. Independent verification: PASS WITH NON-BLOCKING FINDINGS (F-L1–F-I4). Outcomes: FloorplanViewer direct auth.getUser removed from write paths; direct platform Supabase and floorplan table writes removed; direct persistence useMutation blocks removed; direct Storage mutation orchestration (uploadFloorplanModel/deleteFloorplanStorage) removed; direct floorplanKeys ownership removed. Six table-write primitives established in floorplanWrite (create/delete floorplan_models with select/single on create; create/delete floorplan_annotations; create/delete floorplan_measurements with measurement_type distance; throw Supabase errors unchanged; no Auth/React/QC/toast/logger/Storage). useFloorplanViewerMutations established as write Auth + mutation + Storage orchestration + floorplan invalidation + toast + isUploading authority (auth.getUser at mutation time for create model/annotation/measurement only; exact missing-user strings You must be signed in to upload models. / You must be signed in; Storage-before-DB create with best-effort cleanup swallowing cleanup rejection and rethrowing original DB error; Storage-before-DB delete; refreshModels → byProject + toast.info Refreshed; no estimateQueryOptions/room-estimate/financials). FloorplanViewer retains read query factories, estimateQueryOptions read, syncTagsToEstimate product-estimate get/set/invalidate, scene, dialogs, exports, validation, confirm, selection/UI-reset callbacks; public prop projectId and import @/components/floorplan and sole consumer projects.$id.index preserved. Lexical seal tests/invariants/floorplan-viewer-persistence-presentation.invariant.test.ts. Legacy edges FloorplanViewer|@/lib/auth and FloorplanViewer|@/lib/logger retired. Push: successful fast-forward origin/main to b97654d. CI on exact SHA: CI 30399470909 success (ci + invariant-tests); Security 30399470970 success (gitleaks, dependency-audit, server-only-boundary, client-bundle-secret-smoke); Pages 30399469722 success (build/deploy/report-build-status); Vercel success; Supabase Preview failure non-blocking (postgres dial timeout / context deadline exceeded; external connectivity; no schema/migration/RLS/Storage-policy/generated-type change in commit). Local pre-push: focused 22 unit; focused inv 10; full inv 334; UI 516/72; lint/typecheck/build; security boundary 12. Accepted non-blocking: F-L1 no component tests, F-L2 lexical bypasses, F-I1 unstable hook return identities, F-I2 render-closure selection/invalidation parity with HEAD, F-I3 try/catch cleanup restructure equivalent, F-I4 pre-close-out pending-verification notes. FLOORPLANVIEWER DIRECT AUTH, PERSISTENCE MUTATION, STORAGE ORCHESTRATION, AND FLOORPLAN INVALIDATION OWNERSHIP EXTRACTION COMPLETE; further FloorplanViewer direct-persistence child not required for those concerns. Adjacent unrepaired: Storage may succeed before DB delete fails; Storage cleanup failure swallowed after model insert failure; no optimistic floorplan rows; no mutation cancellation/retry; estimate tag sync still in component (AO-1H2); product and AI estimate keys remain separate. Deferred outside AO-1H1: AO-1H2 estimate tag sync cache extraction, DealChat residual mutation/QueryClient, BulkPhoto residual QC, dual estimate keys, optional src/lib/auth cleanup. AO-1 remains Active; AO-1E1.1–E1.3 remain Completed; AO-1F1 remains Completed; AO-1G1 remains Completed; AO-1B1/B2/B3.1/B3.2/C1/C2/D1/D2 remain Completed; C5 remains Completed; AO-1H2 not registered.",
    },
  },
];
