...
// P0-1 Financial Path Normalizer and @repo/services Launch Hardening In Progress (P0-1A plan done; P0-1B implement local)
// AO-1 Presentation Layer Owns No Infrastructure Completed (formal close-out AO-1ZB; final child AO-1S1 d3bde9e9 / gov 544f0813; audit AO-1ZA READY FOR AO-1 CLOSURE)
// AO-1S1 Extract Shell Sign-out Ownership Completed (d3bde9e9 + required CI)
...
export const MIGRATION_CANDIDATES: MigrationCandidate[] = [
  ...
  {
    id: "AO-1S1",
    title: "Extract Shell Sign-out Ownership",
    status: "Completed",
    blastRadius: "T1",
    problem:
      "Sidebar and MobileTopBar imported @/lib/auth and called auth.signOut() directly, mixing shell presentation with browser authentication mutation ownership.",
    currentOwner:
      "Presentation: useSignOut (@/features/auth). Infrastructure: signOutSession → frozen auth.signOut. Shell: Sidebar and MobileTopBar click + navigate.",
    targetOwner:
      "Auth feature owns sign-out command; shell retains button chrome and post-success navigate({ to: '/' })",
    dependencies: ["AO-1", "AO-1M6"],
    dependents: [],
    evidence: {
      commit: "d3bde9e9",
      productionImporters: 2,
      notes:
        "AO-1S1 Completed. One child slice of Active AO-1 (does not complete AO-1; does not migrate useAuth.signOut or settings delete-account; does not redesign AuthProvider/cache lifecycle; does not edit frozen src/lib/auth.ts; does not address AO-1Z). Implementation commit d3bde9e9ba0f0d08feca24e6700d769d7c25f2af (parent 7d4ce23ce373f03fbbfc8e32502bb0a15f51185f); subject refactor(auth): extract shell sign-out ownership; 13 files. Independent verification: PASS WITH NON-BLOCKING FINDINGS (F-L1, F-L2, F-I1–F-I8). Outcomes: shell sign-out ownership extracted. Sidebar and MobileTopBar consume useSignOut through @/features/auth. Both retain sign-out control rendering, existing labels and accessibility (Sidebar: Sign out; MobileTopBar: Out / aria-label Sign out), click handling, await signOut(), post-success navigate({ to: '/' }), and existing shell presentation. The presentation hook owns public signOut(): Promise<void>, delegation to private signOutSession, and unchanged success/error propagation; no navigation, QueryClient operations, useMutation, pending/error lifecycle, toast, or product analytics. Private signOutSession owns one no-argument auth.signOut() call, void success, and unchanged thrown-error propagation; no navigation, cache, UI, analytics, or storage cleanup. src/lib/auth.ts remains frozen and unchanged: browser Supabase auth.signOut with no options, currentUser clearing, listener notification, auth:sign_out:attempt and success breadcrumbs, failure logging, Sentry capture, and error rethrow. AuthProvider and auth-query lifecycle remain unchanged; cache isolation remains listener-driven (cancel/remove non-auth queries; set auth-user data to null). No QueryClient operation exists in shell, hook, or infrastructure. Navigation remains component-owned; both shells await sign-out before navigating to \"/\"; no pending/disabled/error UI, toast, retry, analytics, mutationKey, cancellation, or concurrency guard introduced. Adjacent useAuth.signOut and settings delete-account remain outside AO-1S1. Public API: presentation exports useSignOut + UseSignOutResult; root re-exports presentation; signOutSession not public. Lexical seal tests/invariants/shell-auth-signout-ownership.invariant.test.ts. Legacy import baseline shrank only by removing MobileTopBar.tsx|@/lib/auth and Sidebar.tsx|@/lib/auth; Sidebar @/lib/utils retained; useAuth edges retained; frozen-path-allowlists unchanged; baseline growth 0. Direct shell @/lib/auth authorities: 0. Direct shell auth.signOut authorities: 0. Canonical feature presentation authorities: 1. Private feature infrastructure authorities: 1. Frozen lib/auth SDK-wrapper authorities: 1. Push: successful publication origin/main to d3bde9e9 (divergence 0 0). Exact-SHA CI: CI 30444999916 success (ci + invariant-tests); Security 30445000268 success (gitleaks, dependency-audit, server-only-boundary, client-bundle-secret-smoke); Pages 30444998560 success (build, deploy, report-build-status); Vercel success; Supabase Preview success. Database-definition changes: none. Dependency audit: pre-existing debt (50 vulns; 11 low / 20 moderate / 18 high / 1 critical), unchanged; dependency files unchanged; AO-1S1 introduced none; CI dependency-audit success under report-only policy. Local: focused tests 17; focused inv 10; full inv 444; UI 675/92; lint/typecheck/build; security boundary 12. Accepted non-blocking: F-L1 failure navigation protected by source ordering not runtime rejected-click test; F-L2 progressive lexical seal bypasses; F-I1 signOutSession wraps frozen @/lib/auth; F-I2 feature-to-lib edge outside legacy scanner targets; F-I3 no pending UI; F-I4 no error UI; F-I5 no double-click guard; F-I6 shell rejection remains uncaught; F-I7 adjacent useAuth.signOut and settings deletion remain; F-I8 pre-existing dependency debt. SHELL SIGN-OUT OWNERSHIP EXTRACTED; further AO-1S1 child not required. Deferred outside AO-1S1: formal AO-1 programme close-out (AO-1Z). AO-1 remains Active; AO-1M6 remains Completed.",
    },
  },
  {
    id: "P0-1",
    title: "Financial Path Normalizer and @repo/services Launch Hardening",
    status: "In Progress",
    blastRadius: "T1",
    problem:
      "Financials query constructed ROI inputs with embedded defaults, ad hoc camel/snake mapping, and Math.round(roi.roi) precision loss, outside a reusable @repo/services authority.",
    currentOwner:
      "Canonical normalizer: packages/services/src/financial (normalizeFinancialPath / calculateFinancialPath). Calculation: runRoiEngine. Initial consumer: financialsQueryOptions in src/lib/queries/projects.ts.",
    targetOwner:
      "packages/services owns Financials-path input policy and Financials DTO assembly; runRoiEngine remains calculation authority; financialsQueryOptions retains project/estimate fetch and query-key ownership",
    dependencies: ["AO-1"],
    dependents: [],
    evidence: {
      productionImporters: 1,
      notes:
        "P0-1 In Progress. Planning P0-1A completed (READY FOR P0-1B). Implementation P0-1B local (uncommitted). Canonical owner: packages/services/src/financial. Public API: normalizeFinancialPath + calculateFinancialPath via @repo/services. Initial consumer: src/lib/queries/projects.ts financialsQueryOptions. Verified problems addressed: F-P0-1 implicit Financials defaults centralized as FINANCIALS_PATH_DEFAULT_POLICY (Average, London, 0 holding, 0 rental, timelineWeeks 8) with meta.usedDefaults; F-P0-2 ROI precision — no Math.round(roi.roi), preserve engine one-decimal; F-P0-4 shape mapping — single normalizer for Financials path only. mid_total remains Financials refurb authority. Query keys unchanged: projectKeys.estimateByProject = [projects, id, estimate]; room-estimate not reintroduced. Invariant: tests/invariants/financial-path-normalizer.invariant.test.ts (progressive lexical). Deferred P1+: cross-product holding-cost policy; timeline derivation from estimate; deal-analysis/scoreDeal/pitch/studies/feasibility migration; AI normalizer alignment; package publishing; dependency remediation. Not a child of AO-1; AO-1 remains Completed. P0-1 not Completed until independent verification + commit/push gates.",
    },
  },
];
...
export const ARCHITECTURE_OBJECTIVES: ArchitectureObjective[] = [
  {
    ...
    status: "Completed",
    description:
      "AO-1 Completed. Bounded objective: presentation and route surfaces must not directly own Supabase clients, realtime channels, or persistence and mutation infrastructure targeted by the AO-1 programme; those responsibilities sit behind approved feature/core public APIs, canonical hooks, server functions, repositories, infrastructure adapters, or explicitly governed transitional boundaries. Supersedes former single-migration framing of C8. Formal close-out audit AO-1ZA: READY FOR AO-1 CLOSURE. Formal closure implementation: AO-1ZB. Final child: AO-1S1 Extract Shell Sign-out Ownership. Final child implementation: d3bde9e9ba0f0d08feca24e6700d769d7c25f2af. Final child governance: 544f0813817b945969d54df43ddd59b1f9b81177. Registered unresolved AO-1 children: 0. Unmanaged route residuals: 0. Unmanaged presentation residuals: 0. Unmanaged AO-1 programme residuals: 0. Unapproved frozen-layer growth: 0. New ungoverned legacy edges: 0. Child chronology: C3 channel lifecycle Completed at 0d6fc84; C8 Reclassified into AO-1; AO-1B1 marketplace favorites Completed at 322156a; AO-1B2 quote-request creation Completed at fcc13b6; AO-1B3.1 marketplace message send Completed at fa12ccc; AO-1B3.2 MessagingInbox Realtime lifecycle Completed at d407cc6; AO-1C1 PhotoAnalysisViewer write extraction Completed at 0802bcc; AO-1C2 PhotoAnalysisViewer Apply-to-Estimate cache extraction Completed at fe28f25; AO-1D1 Admin Metrics Read Extraction Completed at d3cab3f; AO-1D2 Dashboard Onboarding Auth Extraction Completed at 9b4da54; AO-1E1.1 AuthExperience Password Credential Extraction Completed at 8bdf817; AO-1E1.2 AuthExperience OAuth Extraction Completed at ce4384a; AO-1E1.3 AuthExperience Magic Link and Password Recovery Extraction Completed at ba442e3; AO-1F1 Auth Callback Auth and QueryClient Ownership Extraction Completed at 68a3eb7; AO-1G1 EstimateBuilder Save Mutation and QueryClient Ownership Extraction Completed at 2e77407; AO-1H1 FloorplanViewer Auth and Persistence Mutations Extraction Completed at b97654d; AO-1H2 FloorplanViewer Estimate Tag Sync Cache Extraction Completed at 6bde476; AO-1I1 BulkPhotoUpload Residual QueryClient Invalidation Extraction Completed at 0f07376; AO-1J1 DealChat Residual Mutation and QueryClient Ownership Extraction Completed at fd1b964; AO-1K1 Unify Estimate Query-Key Authority Completed at 8fce9bc; AO-1L1 Extract AIEstimateBuilder Save-Path Ownership Completed at f316c53; AO-1M1 residual audit completed (programme audit, not a committed candidate; residuals discharged via AO-1M2–AO-1M6 and AO-1S1); AO-1M2 Extract PitchDeckGenerator Persistence and Cache Ownership Completed at 806cdbe; AO-1M3 Extract Gallery Upsert Mutation Ownership Completed at 409eb037; AO-1M4 Extract Project Stage Mutation Ownership Completed at 5107f2ec; AO-1M5 Extract Deal Opportunity Update Mutation Ownership Completed at fd769094; AO-1M6 Extract DealAnalysisCard Mutation Ownership Completed at e9d374c; AO-1S1 Extract Shell Sign-out Ownership Completed at d3bde9e9 (governance close-out 544f0813). Direct infrastructure ownership targeted by AO-1 has been removed from governed presentation and route surfaces or placed behind approved feature/core public APIs and explicitly documented boundaries. Legacy-import baseline decreased during the programme (shrink-only; growth prohibited). Frozen src/lib, src/hooks, and src/services allowlists remain non-expanding transitional layers (src/services README-only). Required ownership, public-API, legacy, freeze, authentication, query-lifecycle, security, presentation-boundary, and shell sign-out invariants remain active (full suite 444 passed at AO-1ZA; UI 675/92; build passed; security boundary 12). Accepted transitional boundaries retained: settings delete-account → useAuth().signOut() → frozen auth.signOut() (outside AO-1S1; optional future auth cleanup); progressive lexical rather than AST enforcement; feature-to-lib edges outside some legacy scanner target directories; wildcard public feature re-exports. Out of AO-1 scope retained: DealIntakeForm deterministic analyzeDeal; gallery cover upload helper; P0 launch hardening; package hygiene; dependency remediation (50 baseline-equivalent findings; dependency files unchanged by AO-1 closure); broader product work; universal architecture convergence; complete authentication migration; deletion of frozen transitional layers. Governance Supabase Preview on 544f0813 failed before schema/migration validation due to external Postgres dial timeout; accepted non-blocking for two-file governance-only commit (not classified successful). Closure commit: 018139ba057606b8cb13e466047660fdd52a4714 (parent 544f0813817b945969d54df43ddd59b1f9b81177; subject chore(architecture): complete AO-1 programme; two governance files). AO-1ZE exact-SHA verification completed for 018139ba057606b8cb13e466047660fdd52a4714. CI 30449085919 succeeded, including ci (90566530693) and invariant-tests (90566530788). Security 30449087226 succeeded, including gitleaks (90566535217), report-only dependency-audit (90566535238), server-only-boundary (90566535244), and client-bundle-secret-smoke (90566535273). GitHub Pages 30449083267 succeeded, including build (90566526701), deploy (90566666868), and report-build-status (90566666922). Vercel deployment for refurb-geniusv1 succeeded (status 51286896692; deployment completed). Supabase Preview 90568000674 failed because of an external Postgres connection timeout (dial error: context deadline exceeded) before schema or migration validation. The commit contained no database-definition changes. The failure was classified as external and non-blocking, not successful. Dependency findings remained at the pre-existing baseline of 50 (11 low, 20 moderate, 18 high, 1 critical). Dependency files were unchanged and AO-1 introduced none. Further AO-1 migration child required: No.",
    relatedCandidates: [
      "C3",
      "C8",
      "AO-1B1",
