...
// AO-1L1 Extract AIEstimateBuilder Save-Path Ownership In Progress (local implementation pending verification)
...
export const MIGRATION_CANDIDATES: MigrationCandidate[] = [
  ...
  },
  {
    ...
    evidence: {
      productionImporters: 0,
      notes:
        "AO-1L1 In Progress — implemented locally, pending independent verification. One child slice of Active AO-1 (does not complete AO-1; does not extract generation; does not redesign pricing; does not change useSaveAIEstimate invalidation). Pure mapper established: buildAIEstimateBuilderSaveInput (exact title `AI Estimate — ${propertyType}, ${bedrooms} bed`; vat_rate 20; notes || undefined; calculateLineItem(item, multiplier) per item; totals passthrough). Canonical presentation authority: useAIEstimateBuilderSave({ projectId, onSaved }) composes useSaveAIEstimate; empty rooms toast Generate or add rooms first without mutate; success toast Estimate saved then onSaved(result.estimate.id); error toast err.message || Failed to save estimate; isPending passthrough; no useMutation/useQueryClient/projectKeys/invalidateQueries/repository. AIEstimateBuilder retains generation, editor, pricing preview, render; wires saveEstimate snapshot + isSavePending. Public export via presentation barrel → @/features/estimate. Lexical seal tests/invariants/ai-estimate-builder-save-presentation.invariant.test.ts. AO-1K1 invalidation ownership unchanged (useSaveAIEstimate only). Generation path unchanged. Repository/schema/pricing engine unchanged. AO-1 remains Active; AO-1K1 remains Completed. Pending: independent verification, commit, push, CI, close-out.",
    },
  },
];
