...
export const MIGRATION_CANDIDATES: MigrationCandidate[] = [
  ...
  },
  {
    ...
    blastRadius: "T1",
    problem:
      "PhotoAnalysisViewer owns useQueryClient, estimateQueryOptions, setQueryData, and invalidateQueries for Apply-to-Estimate client-only estimate cache mutation in presentation.",
    currentOwner:
    ...
    evidence: {
      productionImporters: 0,
      notes:
        "AO-1C2 In Progress — implemented locally, pending independent verification. Outcomes (local): mapPhotoAnalysesToEstimateRooms pure helper preserves exact room/item/cost/id formulas and category→room quirk for apply path; useApplyPhotoAnalysesToEstimate owns getQueryData/setQueryData/invalidateQueries on estimateQueryOptions only (no useMutation, no toast, no Auth/Supabase, no room-estimate key); PhotoAnalysisViewer removes useQueryClient and estimateQueryOptions; retains selection, empty/success toasts, clear selection, AO-1C1 edit hook; append-only rooms (no name dedupe); dual estimate query keys not unified. Lexical seal tests/invariants/photo-analysis-viewer-apply-estimate-presentation.invariant.test.ts; AO-1C1 write inv no longer requires residual QC. Does not complete AO-1; does not migrate estimate DB save, FloorplanViewer estimate sync, or dual keys.",
    },
  },
];
