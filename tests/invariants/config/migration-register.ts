...
// AO-1M4 Extract Project Stage Mutation Ownership In Progress (local implementation pending verification)
...
export const MIGRATION_CANDIDATES: MigrationCandidate[] = [
  ...
  },
  {
    ...
    evidence: {
      ...
      notes:
        "AO-1M4 In Progress — implemented locally, pending independent verification. One child slice of Active AO-1 (does not complete AO-1; does not extract project create; does not migrate project list/detail reads; does not introduce stage state machine; does not address R-M5–R-M6 opportunity/DealAnalysisCard/shell signOut). Persistence authority: projectStageRepository.setProjectStageDone → projects UPDATE eq id; payload exactly one of photos_done/analysis_done/estimate_done/report_done = value; no select; error throw Error(message); no auth resolution (RLS projects_all_own). Canonical presentation authority: useSetProjectStage() from @/features/projects; mutate({ id, stage, value }); onMutate cancelQueries projectKeys.all + projectKeys.byId(id) exact true; applyProjectStageOptimistic(projectStagePatch); onError restoreProjectStageCaches; no onSuccess/onSettled/invalidate/toast/logger. Routes: projects.$id.upload/analysis/estimate/report import useSetProjectStage from @/features/projects; retain useProject from @/hooks/useProjects. Transitional useSetProjectStage removed from useProjects.ts (file retained for list/detail/create/progress). Lexical seal tests/invariants/project-stage-mutation-presentation.invariant.test.ts; C4c-3 stage dual-cache path retargeted to presentation hook. Query helpers src/lib/queries/projects.ts unchanged. Schema/migrations/RLS/dependency files unchanged. AO-1 remains Active; AO-1M3 remains Completed. Pending: independent verification, commit, push, CI, close-out.",
    },
  },
];
