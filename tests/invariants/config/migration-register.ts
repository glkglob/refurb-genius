...
// AO-1F1 Auth Callback Auth and QueryClient Ownership Extraction In Progress (local)
...
export const MIGRATION_CANDIDATES: MigrationCandidate[] = [
  ...
  },
  {
    ...
    evidence: {
      productionImporters: 0,
      notes:
        "AO-1F1 In Progress — implemented locally, pending independent verification. Outcomes (local): direct exchangeCodeForSession removed from auth_.callback.tsx; direct getSession removed; direct fromSupabaseUser removed; direct AUTH_USER_QUERY_KEY setQueryData removed; platform Supabase import removed; @/lib/auth import removed; useQueryClient removed from route. exchangeAuthCode established (exchangeCodeForSession(code); throw returned errors; return { user }). getBrowserAuthSession established (getSession; session → { user }; no session → null; ignore returned error field; rejections propagate). resolveAuthCallbackDestination established (exact startsWith('/') ? redirectTo : '/dashboard'; preserves protocol-relative and /auth acceptance). completeAuthCallback established (urlError → error_description ?? error; no-code session map; code exchange; type===recovery without seed; map via fromSupabaseUser; destination resolve; exchange failures → error result; getSession rejections propagate). useAuthCallbackCompletion established (seed AUTH_USER_QUERY_KEY before navigate on authenticated; recovery navigate /auth?mode=reset replace true without seed; error → { ok:false, error }). Route retains zod search (code,type,error,error_description,redirect_to), loading Completing sign in…, error Authentication failed + back link, effect without didRun/abort/retry. src/lib/auth.ts unchanged. AuthExperience unchanged. AUTH_USER_QUERY_KEY authority remains @/hooks/useAuth. Lexical progressive seal tests/invariants/auth-callback-presentation.invariant.test.ts. Does not complete AO-1; does not harden open-redirect; does not add recovery cache seed. AO-1 remains Active; AO-1E1.1/E1.2/E1.3 remain Completed.",
    },
  },
];
