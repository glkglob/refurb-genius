...
// AO-1G1 EstimateBuilder Save Mutation and QueryClient Ownership Extraction In Progress (implemented locally, pending verification)
...
export const MIGRATION_CANDIDATES: MigrationCandidate[] = [
  ...
  },
  {
    ...
    currentOwner:
      "Save-input mapping: buildEstimateBuilderSaveInput. Manual-save mutation + product estimate cache + toasts + draft clear + onSaved: useSaveEstimateBuilder. Persistence: saveAIEstimate (unchanged). Product estimate key: estimateQueryOptions / projectKeys.estimateByProject. Editor state, calculations, PDF: EstimateBuilder.",
    targetOwner:
    ...
    evidence: {
      ...
      productionImporters: 0,
      notes:
        "AO-1G1 In Progress — implemented locally, pending independent verification. One contained child of Active AO-1 (does not complete AO-1; does not consolidate dual estimate query keys; does not migrate AIEstimateBuilder/useSaveAIEstimate). Outcomes pending verification: direct useMutation/useQueryClient/saveAIEstimate/getQueryData/setQueryData/invalidateQueries/cancelQueries/estimateQueryOptions/projectKeys removed from EstimateBuilder; buildEstimateBuilderSaveInput established as pure payload authority (title `${name||Property} Refurbishment Estimate`; vat_rate 20; notes Manual estimate built with drag & drop builder; item labour/materials/weeks 0; is_ai_suggested false in input); useSaveEstimateBuilder established as canonical manual-save authority (saveAIEstimate; onMutate cancel+snapshot+optimistic product key with editor item spread + base_unit_cost; onError truthy previous rollback + toast Save failed; onSuccess estimateByProject then financialsByProject invalidate, localStorage remove estimate-draft, toast Estimate saved, onSaved; getSeededEstimate one-shot product key only; no room-estimate key). Public props projectId/project/onSaved and consumers projects.$id.estimate + projects.$id.index unchanged. Lexical seal tests/invariants/estimate-builder-save-presentation.invariant.test.ts. Legacy edge EstimateBuilder|@/lib/queries/projects retired. Deferred outside AO-1G1: dual estimate query keys, AIEstimateBuilder/useSaveAIEstimate room-estimate invalidation-only path, DealChat residual QC, FloorplanViewer multi-table mutations and estimate sync, BulkPhoto residual QC, optional src/lib/auth cleanup, draft localStorage write restoration, repository AI flag semantics. AO-1 remains Active; AO-1E1.1–E1.3 remain Completed; AO-1F1 remains Completed.",
    },
  },
];
