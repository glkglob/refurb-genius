...
// AO-1I1 BulkPhotoUpload Residual QueryClient Invalidation Ownership Extraction In Progress
...
export const MIGRATION_CANDIDATES: MigrationCandidate[] = [
  ...
  },
  {
    ...
    evidence: {
      productionImporters: 0,
      notes:
        "AO-1I1 In Progress — implemented locally, pending independent verification. One child slice of Active AO-1 (does not complete AO-1; does not redesign Bulk orchestration; does not migrate photos-write or query factories). Outcomes (local): useInvalidateProjectPhotos established as canonical project-photo list invalidation authority (useQueryClient; projectKeys.photosByProject(projectId) → [projects, projectId, photos]; void fire-and-forget invalidateQueries; no exact; no await; no setQueryData/getQueryData/cancelQueries; no useMutation/pending; no Supabase/Storage/upload). BulkPhotoUpload: useQueryClient removed; direct invalidateQueries removed; projectKeys and @/lib/queries/projects import removed; invalidateProjectPhotos() called after completed item patches on full success before toast.success Upload complete.; on PhotoUploadBatchError with successes.length > 0 after success/failure patches before toast.error N file(s) failed; never on total failure/unknown error; uploadProjectPhotos remains component-owned with concurrency 3; progress/processingRef/isProcessing/toasts/selection/validation retained; public prop projectId and @/components/BulkPhotoUpload consumer projects.$id.index preserved. C5 photos-query-keys write seal intact. Lexical seal tests/invariants/bulk-photo-upload-queryclient-presentation.invariant.test.ts. Legacy edge BulkPhotoUpload.tsx|@/lib/queries/projects retired; photos-write and photos-types edges retained. No commit/push yet. AO-1 remains Active; AO-1H1/H2 remain Completed; adjacent: no cancellation/retry/optimistic rows; batch concurrency remains 3; invalidation rejection non-blocking; bulk orchestration remains component-owned.",
    },
  },
];
