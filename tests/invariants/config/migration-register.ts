...
// AO-1M2 Extract PitchDeckGenerator Persistence and Cache Ownership In Progress (local implementation pending verification)
...
export const MIGRATION_CANDIDATES: MigrationCandidate[] = [
  ...
  },
  {
    ...
    evidence: {
      productionImporters: 0,
      notes:
        "AO-1M2 In Progress — implemented locally, pending independent verification. One child slice of Active AO-1 (does not complete AO-1; does not server-migrate PDF upload; does not redesign PDF layout; does not address R-M2–R-M6). Pure PDF authority retained: generatePitchDeckPDF via legacyPdfExporter. Persistence authority retained: supabaseExportRepository.savePitchDeckExport → savePitchDeckToSupabase (pitch-decks bucket path `${userId}/${projectId}/${filename}`; contentType application/pdf upsert false; pitch_deck_exports insert project_id/created_by/storage_path/metadata; rollback remove on insert failure). Canonical presentation authority: useGenerateAndSavePitchDeck({ projectId, project? }) → { generatePitchDeck, isPending, progress, progressStage }; auth.getUser gate toast You must be signed in to generate a pitch deck; six-query Promise.all with project prop short-circuit; estimateQueryOptions (AO-1K1); generate with includePhotos/include3D/includeSensitivity true; download before optional save; toast.success Pitch deck generated then optional save; await invalidate pitchDecksByProjectQueryOptions queryKey [projects, projectId, pitchDecks] after save only; toast.success Saved to your project; save failure non-fatal toast.info; outer failure toast.error Failed to generate pitch deck; 600ms delayed pending reset. PitchDeckGenerator retains button chrome, progress UI, trigger variants; imports only useGenerateAndSavePitchDeck from @/features/export. Lexical seal tests/invariants/pitch-deck-generator-persistence-presentation.invariant.test.ts. Gallery/stage/opportunity/DealAnalysisCard/shell signOut residuals unchanged. AO-1 remains Active; AO-1L1 remains Completed. Pending: independent verification, commit, push, CI, close-out.",
    },
  },
];
