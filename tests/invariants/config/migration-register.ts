...
export const MIGRATION_CANDIDATES: MigrationCandidate[] = [
  ...
  },
  {
    ...
    evidence: {
      productionImporters: 0,
      notes:
        "AO-1H1 In Progress — implemented locally, pending independent verification. One child slice of Active AO-1 (does not complete AO-1; does not extract estimate tag sync; does not register AO-1H2). Outcomes (local): floorplanWrite established as table-write authority (create/delete floorplan_models, floorplan_annotations, floorplan_measurements; accept resolved userId; throw Supabase errors unchanged; no React/QC/toast/Auth/Storage). useFloorplanViewerMutations established as Auth + mutation + Storage orchestration + floorplanKeys invalidation + toast + isUploading authority (auth.getUser at mutation time for create model/annotation/measurement only; exact missing-user strings; Storage-before-DB create order with best-effort cleanup; Storage-before-DB delete order; measurement_type distance; refreshModels → byProject + toast.info Refreshed). FloorplanViewer: direct auth/Supabase/useMutation/uploadFloorplanModel/deleteFloorplanStorage/floorplanKeys removed; public prop projectId and @/components/floorplan import preserved; syncTagsToEstimate and estimate QueryClient retained intentionally; read query factories retained. Lexical seal tests/invariants/floorplan-viewer-persistence-presentation.invariant.test.ts. Legacy edges FloorplanViewer|@/lib/auth and FloorplanViewer|@/lib/logger retired. No commit/push yet. AO-1 remains Active; AO-1G1 remains Completed; AO-1H2 not started.",
    },
  },
];
