...
// AO-1J1 DealChat Residual Mutation and QueryClient Ownership Extraction In Progress (local implementation)
...
export const MIGRATION_CANDIDATES: MigrationCandidate[] = [
  ...
  },
  {
    ...
    evidence: {
      ...
      productionImporters: 0,
      notes:
        "AO-1J1 In Progress. One child slice of Active AO-1 (does not complete AO-1; does not redesign AI/streaming/schema; does not change C3 channel lifecycle). Local implementation (pending independent verification, commit, push, CI). Outcomes intended: DealChat direct useMutation removed; direct useQueryClient removed; direct cancelQueries/getQueryData/setQueryData/invalidateQueries removed; inline dealChatKeys removed to shared dealChatKeys factory (exact [deal-threads, oppId] and [deal-messages, threadId]). Canonical authorities: useSendDealChatMessage (await cancel → snapshot → opt-${Date.now()} user append → sendMessageServerFn → strip opt-* + append user+assistant; rollback context.prev; trackEvent deal_message_sent; no send invalidation; onOptimisticClearDraft); useCreateDealThread (createThreadServerFn with title from component Thread N; void invalidate threads; onCreated selection; trackEvent deal_thread_created); useInvalidateDealMessages (void fire-and-forget messages invalidate for C3 onInsert). Public prop opportunityId, import @/components/deal-copilot/DealChat, sole consumer deal-copilot/$opportunityId preserved; no public callbacks. Reads listThreadsServerFn/listMessagesServerFn remain component-owned. C3 useDealMessagesChannel unchanged. Lexical seal tests/invariants/deal-chat-mutation-queryclient-presentation.invariant.test.ts. Legacy: DealChat|@/lib/analytics retired; DealChat|@/core/dealCopilot/presentation added; realtime edge retained. Pending: independent verification, commit, push, exact-SHA CI/Security/Pages/Vercel/Supabase. AO-1 remains Active; AO-1I1 remains Completed; C3 remains Completed.",
    },
  },
];
