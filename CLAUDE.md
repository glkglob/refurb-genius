...
5. **Migrations must be idempotent.** All `CREATE POLICY` (and recent `CREATE TABLE`/
   `CREATE INDEX` / `ADD COLUMN`) statements use `IF NOT EXISTS` guards or the
   `DO $$ BEGIN IF NOT EXISTS (SELECT FROM pg_policies ...) THEN CREATE POLICY ...`
   pattern. This prevents "already exists" errors during `supabase db push`,
   `supabase db reset`, re-deploys, or when replaying on existing DBs.
   Never add bare `create policy "..."` in new migrations.
...
`vercel` preset.
Vercel install is locked with `--frozen-lockfile` (see vercel.json) and package.json
has `"packageManager": "pnpm@9.15.9"` + cleaned pnpm-workspace.yaml + .npmrc for
stable lockfile (no more pnpmfileChecksum mismatches).
...
- See Phase 1 fixes (pnpmfileChecksum removal, .pnpmfile.cjs deleted, onlyBuiltDependencies + packageManager for stability).

Vercel + CI are now pinned for reproducible lockfile behavior.
