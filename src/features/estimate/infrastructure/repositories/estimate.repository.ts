/**
 * Estimate slice — Supabase persistence (browser context).
 *
...
export class SupabaseEstimateRepository implements EstimateRepositoryPort {
  async saveProjectEstimate(
    projectId: string,
    result: PricingEngineResult,
  ): Promise<SavedEstimateRef> {
    const persisted = await saveProjectEstimate(projectId, result);
    return { estimateId: persisted.estimate.id };
  }
}

export const supabaseEstimateRepository: EstimateRepositoryPort = new SupabaseEstimateRepository();
