/**
 * Estimate slice — OpenAI estimate-generation adapter (server-only).
 * Moved from `src/core/ai/server/openAiEstimate.server.ts` (now a shim).
...
import { aiEstimateResponseSchema, safeParseEstimate } from "@/core/ai/validation";
import { withRetry } from "@/core/ai/platform/retry";
import type {
  AIGeneratedItem,
  AIGeneratedRoom,
  GenerateEstimateInput,
} from "../../domain/aiEstimate.types";

export type { AIGeneratedItem, AIGeneratedRoom, GenerateEstimateInput };
